<script>
		$(document).ready(function($) {
				...
				//KORISNIČKO IME TRAZI U BAZI 
				$("#korisnik").focusout(function() {

						$.ajax({
							type : "POST",
							url : "provjeriKorisnickoImePromjena.php",
							data : "korisnik=" + $('#korisnik').val() + "&sifraKorisnika=" + <?php echo $sifra; ?>,
							cache : false,
							success : function(data) {
								//alert(data);
								if (data == "POSTOJI") {
									$('#korisnik').tooltipster('content', 'Korisničko ime se koristi.');
				...
				$("#oib").focusout(function() {
						...
						$.ajax({
							type : "POST",
							url : "provjeriOibPromjena.php",
							data : "oib=" + $('#oib').val() + "&sifraKorisnika=" + <?php echo $sifra; ?>,
							cache : false,
							success : function(data) {
								//alert(data);
								if (data == "POSTOJI") {
	...
	$("#promijeni").click(function(){
				...
				$("label[for=ime]").click(function(){
					$('#ime').val($('input:hidden[name=hidIme]').val());
					$('#ime').tooltipster('hide');
				});
				
				$("label[for=prezime]").click(function(){
					$('#prezime').val($('input:hidden[name=hidPrezime]').val());
					$('#prezime').tooltipster('hide');
				});
				
				$("label[for=oib]").click(function(){
					$('#oib').val($('input:hidden[name=hidOib]').val());
					$('#oib').tooltipster('hide');
				});
				
				$("label[for=datumRodenja]").click(function(){
					$('#datumRodenja').val($('input:hidden[name=hidDatumRodenja]').val());
					$('#datumRodenja').tooltipster('hide');
				});
				...
				$("label[for=brojClanovaObitelji]").click(function(){
					$('#brojClanovaObitelji').val($('input:hidden[name=hidBrojClanovaObitelji]').val());
					$('#brojClanovaObitelji').tooltipster('hide');
				});
				
				$("label[for=korisnik]").click(function(){
					$('#korisnik').val($('input:hidden[name=hidKorisnik]').val());
					$('#korisnik').tooltipster('hide');
				});
